...
import { isEmpty, isNil, prepareOutput } from './export-helpers';
import { ColumnDefinition, ExportState } from './core-interfaces';
import { LocationExportFormats, LocationExportWorkerPayload } from './payload-interfaces';

export const exportLocations = (payload: LocationExportWorkerPayload) => {
  const state = new LocationExportState(payload);
  return prepareOutput(state, payload.outputType);
};

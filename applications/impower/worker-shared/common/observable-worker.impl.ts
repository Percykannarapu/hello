...
export class ObservableWebWorker<TPayload, TResult>
  ...
  private setup(observer: Subscriber<WorkerResponse<TResult>>) {
    this.workerInstance.onmessage = ({data}: { data: WorkerResponse<TResult> }) => {
      switch (data.status) {

...
export class TargetAudienceService implements OnDestroy {
  ...
  private static matchProjectVar(projectVar: ImpProjectVar, audience: AudienceDataDefinition) : boolean {
    ...
    return sourceType === audience.audienceSourceType && sourceName === audience.audienceSourceName && id.toString() === audienceIdentifier;
  }
  ...
  public updateProjectVars(audience: AudienceDataDefinition) {
    ...
    for (const projectVar of this.projectVarService.get()) {
      if (TargetAudienceService.matchProjectVar(projectVar, audience)) {
        this.projectVarService.update(projectVar, newProjectVar);
      }
    }
  }

  private removeProjectVar(sourceType: 'Online' | 'Offline' | 'Custom' | 'Combined' | 'Combine/Convert' | 'Composite', sourceName: string, audienceIdentifier: string) {
    for (const projectVar of this.projectVarService.get()) {
  ...
  public removeAudience(sourceType: 'Online' | 'Offline' | 'Custom' | 'Combined' | 'Combine/Convert' | 'Composite', sourceName: string, audienceIdentifier: string) : void {
  ...
  public addDeletedAudience(sourceType: 'Online' | 'Offline' | 'Custom' | 'Combined' | 'Combine/Convert' | 'Composite', sourceName: string, audienceIdentifier: string) : void {
    const sourceId = this.createKey(sourceType, sourceName);
    const audienceId = this.createKey(sourceId, audienceIdentifier);
    if (this.audienceMap.has(audienceId)) {

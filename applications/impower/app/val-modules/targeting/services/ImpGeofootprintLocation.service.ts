...
//          console.log(`Found match, group ${groupIndex}: ${match}`);
   ...
   public exportStore(filename: string, exportFormat: EXPORT_FORMAT_IMPGEOFOOTPRINTLOCATION, project: ImpProject, isDigital?: boolean, filter?: (loc: ImpGeofootprintLocation) => boolean, exportType?: string)
   {
      ...
      const siteType =  exportType && exportType.toLocaleUpperCase() === 'SITES' ? 'Site' : 'Competitor';
      this.isMarket = this.get().some(loc => loc.clientLocationTypeCode === siteType &&  ( (loc.marketName != null && loc.marketName !== '') || (loc .marketCode != null && loc.marketCode !== '')));

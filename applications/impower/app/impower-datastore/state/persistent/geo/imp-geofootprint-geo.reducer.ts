...
import { ImpGeofootprintTradeAreaActionsAffectingChildren, ImpGeofootprintTradeAreaActionTypes } from '../tradeArea/imp-geofootprint-trade-area.actions';
...
export function reducer(state = initialState, action: reducerActions) : State {
  switch (action.type) {
    ...
    case PersistentActionTypes.EntityCreateSuccessful:
      return adapter.setAll(action.payload.normalizedEntities.impGeofootprintGeos, state);

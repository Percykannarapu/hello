...
import { EntityCreateSuccessful, EntityLoadSuccessful, ImpGeofootprintLocationActionsAffectingParent, ImpGeofootprintLocationActionTypes, PersistentActionTypes } from '..';
...
export function reducer(state = initialState, action: reducerActions) : State {
  switch (action.type) {
    ...
    case PersistentActionTypes.EntityLoadSuccessful:
      return adapter.addAll(action.payload.normalizedEntities.impGeofootprintMasters, state);

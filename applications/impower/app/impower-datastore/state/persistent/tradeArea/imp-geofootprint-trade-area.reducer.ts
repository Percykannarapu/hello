...
import { ImpGeofootprintGeoActionsAffectingParent, ImpGeofootprintGeoActionTypes } from '../geo/imp-geofootprint-geo.actions';
...
export function reducer(state = initialState, action: reducerActions) : State {
  switch (action.type) {
    ...
    case PersistentActionTypes.EntityLoadSuccessful:
      return adapter.setAll(action.payload.normalizedEntities.impGeofootprintTradeAreas, state);

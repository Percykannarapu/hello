...
export class EsriRendererEffects {
  ...
  enableShading$ = this.actions$.pipe(
    ...
    tap(([action, mapState]) => this.rendererService.setShadingRenderer(mapState, action.payload.data, action.payload.isNumericData, action.payload.statistics,action.payload.legend))
  );
